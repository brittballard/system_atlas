...
describe TeamsController do
  ...
  describe 'create' do
    before do
      ...
      @entity_definition = mock_model(Team)
      @entity.stub!(:entity_definition).and_return(@entity_definition)
      Entity.stub!(:load_entity_for_save).with(an_instance_of(Team), @user).once.and_return(@entity)
      @entity_definition.stub!(:valid?).and_return(true)
      @entity.stub!(:save).and_return(true)
      post :create, { :team => FactoryGirl.attributes_for(:team).merge({ :entity_attributes => { :system_identifier => '' } }) }
    end
